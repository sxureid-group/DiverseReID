...
import copy
import pdb
from reid.utils.data.utils import mkdir_if_missing, write_json, read_json
...
class CUHK03(object):
    ...
    def __init__(self, root='data', split_id=0, cuhk03_labeled=True, cuhk03_classic_split=False, **kwargs):
        ...
        self.imgs_detected_dir = osp.join(self.data_dir, 'images_detected')
        self.imgs_labeled_dir = osp.join(self.data_dir, 'images_labeled')

        self.split_classic_det_json_path = osp.join(self.data_dir, 'splits_classic_detected.json')
        self.split_classic_lab_json_path = osp.join(self.data_dir, 'splits_classic_labeled.json')

        self.split_new_det_json_path = osp.join(self.data_dir, 'splits_new_detected.json')
        self.split_new_lab_json_path = osp.join(self.data_dir, 'splits_new_labeled.json')
        ...
        for i in range(len(train)):#fpath,pid,camid pid:[0,766]
            # train[i][0] = train[i][0].split('data/')[-1] 改为下面这个
            train[i][0] = osp.join(root, train[i][0].split('data/')[-1])
            train[i][2] = train[i][2]-1
        ...
        mix = copy.deepcopy(query) + copy.deepcopy(gallery)
        for i in range(len(mix)):
            pid = mix[i][1]
            pid_container.add(pid)
        pid2label = {pid: label+767 for label, pid in enumerate(pid_container)} #pid:[767,1466]
        a = set()
        for i in range(len(mix)):
            # mix[i][0] =  mix[i][0].split('data/')[-1] 改为下面这个
            mix[i][0] = osp.join(root, mix[i][0].split('data/')[-1])
            mix[i][1] = pid2label[mix[i][1]]
            mix[i][2] = mix[i][2]-1
            a.add(mix[i][1])
            mix[i] = tuple(mix[i])

        mix_dataset = train + mix
        p =[]
        c = []
        for i in range(len(mix_dataset)):
            p.append(mix_dataset[i][1])
            c.append(mix_dataset[i][2])
        self.num_mix_cams = len(set(c))
        ...
        print("  mix      | {:5d} | {:8d}".format(len(list(set(p))), len(p)))
        print("  ------------------------------")

        for i in range(len(query)):
            # query[i][0] = query[i][0].split('data/')[-1]
            query[i][0] = osp.join(root, query[i][0].split('data/')[-1])
            query[i]=tuple(query[i])

        for i in range(len(gallery)):
            # gallery[i][0] =  gallery[i][0].split('data/')[-1]
            gallery[i][0] = osp.join(root, gallery[i][0].split('data/')[-1])
        ...
        self.num_mix_pids = len(list(set(p)))
