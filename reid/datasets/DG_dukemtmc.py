...
class DukeMTMC(BaseImageDataset):
    ...
    def __init__(self, root, verbose=True, **kwargs):
        ...
        mix_dataset = self._process_mix_dir([self.train_dir, self.query_dir, self.gallery_dir], relabel=True) # 哈哈，被我发现了，是把train 和test一起训练
        if verbose:
            print("=> DukeMTMC-reID loaded")
            self.print_dataset_statistics(train, query, gallery, mix_dataset)
    ...
    def _process_mix_dir(self, dir_path_list, relabel=False):
        img_paths = []
        for dir_path in dir_path_list:
            img_paths = img_paths + glob.glob(osp.join(dir_path, '*.jpg'))
